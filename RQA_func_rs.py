...
inst_name = 'mix_test_pm10'
inst_name_n = 'mix_test_pminf'
inst_name_m = 'mix_test_pinf'

FLS_txt = 'file_list.txt'

total_path = wav_folder + sls + inst_name + sls + FLS_txt
total_path_n = wav_folder + sls + inst_name_n + sls + FLS_txt
total_path_m = wav_folder + sls + inst_name_m + sls + FLS_txt
...
with open(total_path_n) as fp:
    for line in fp:
        open_ct += 1
        fileN_n.append(line[:-1])

with open(total_path_m) as fp:
    for line in fp:
        open_ct += 1
        fileN_m.append(line[:-1])
